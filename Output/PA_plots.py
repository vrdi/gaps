...
df = pd.read_csv("../PA_Viz/Metrics/BPOP_2000.csv", header=None)
for x in range(4000,100001,2000):
    df = pd.concat([df, pd.read_csv("../PA_Viz/Metrics/BPOP_{0}.csv".format(x), header=None)], ignore_index=True)
...
# plt.ylabel("BPOP")
# plt.savefig("pa_bpop_ensemble.png")
# plt.show()
...
plt.hist(left_district, bins=[x - .5 for  x in range(1,19)])
