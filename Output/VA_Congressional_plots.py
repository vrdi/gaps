...
df = pd.read_csv("../VA_Viz/Full_State/Congressional/DOJBVAP_2000.csv", header=None)
for x in range(4000,100001,2000):
    df = pd.concat([df, pd.read_csv("../VA_Viz/Full_State/Congressional/DOJBVAP_{0}.csv".format(x), header=None)], ignore_index=True)
...
#plt.ylabel("DOJ BVAP")
#plt.savefig("va_bvap_congressional.png")
#plt.show()
...
plt.savefig("va_bvap_gap_congressional.png")
plt.show()

plt.scatter(gaps, left_district)
plt.ylabel("Left district")
plt.xlabel("Size of BVAP gap")
plt.savefig("va_bvap_size_district_congressional.png")
plt.show()
...
plt.savefig("va_bvap_gap_values_congressional.png")
plt.show()

plt.hist(left_district, bins=[x - .5 for  x in range(1,12)])
plt.xlabel("Left district")
plt.ylabel("Frequency")
plt.savefig("va_bvap_gap_district_congressional.png")
plt.show()
